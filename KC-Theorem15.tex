...
\begin{proof}
  ...
  Let $L$ be the language defined similarly to Problem~1 in Homework~7.
  Namely, upon input $x$ of length $n \coloneqq |x|$, let $C$ be the
  lexicographically first circuit of size $n^{\log^2 n}$ which is not computed
  by any circuit of size $n^{\log n}$.
  % TODO: argue for existence
  We obtain that $L \in \class{QP}^{\class{\Sigma}^p_k}$ for some $k \in \N$.
  % TODO: show this! k = 4?
  Now, assume $L \in \Ppoly$. In particular, assume $L$ is decided by a
  $n^a$-size circuit family.
  % TODO: elaborate...
  However, since $n^a \in o(n^{\log n})$, the circuits in this family cannot
  compute $L$ for large enough $x$ anymore.
\end{proof}
