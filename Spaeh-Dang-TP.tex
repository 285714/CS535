...
\begin{proof}
	Idea: From Theorem \ref{BPPimplication}, we know that if $\mathprob{MCSP}$ is $\NP$-hard under a natural reduction from $\mathprob{SAT}$, then $\BPP \subseteq \class{SUBEXP}$. Thus, we can diagonalize against $\class{SUBEXP}$ with a Turing Machine $M$ in $E$ and $M$ should mess up when the input length is large enough.
\end{proof}
