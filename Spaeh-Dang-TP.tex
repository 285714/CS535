...
\begin{theorem}[\cite{10.1145/335305.335314}]
  If $\mathprob{MCSP}$ is $\NP$-hard under a natural reduction from
  $\mathprob{SAT}$, then $\class{E}$ contains a family of Boolean functions
  $f_n$ of circuit complexity $2^{\Omega(n)}$ (i.o.), unless
  $\NP \subseteq \class{SUBEXP}$
\end{theorem}
...
\begin{proof}
  ...
  The difference is that here, we show that a less generous bound on
  ...
  So, to the contrary, assume that $s_n \in O(n^\varepsilon)$ for every
  $\varepsilon > 0$.
  ...
  $O(2^{n^{2 \varepsilon}})$ circuits of size at most $s_n$ for every
  $\varepsilon > 0$.
  ...
  It therefore follows that we can decide the satisfiability of $\phi$
  in subexponential time, i.e. $\SAT \in \class{SUBEXP}$, and we have
  recreated the contradiction.

  Now, [Too tired, will finish tomorrow]
\end{proof}
