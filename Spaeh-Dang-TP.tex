...
learning theory:
If we want to know what hidden Boolean function produced a certain set of
...
As for $\mathprob{MCSP}$ and the class $\P$, it is very likely that $\mathprob{MCSP} \notin \P$ mainly because it will put an end to cryptography as briefly mentioned in Section \ref{subsect:MCSP-P}. One thing we want to emphasize that if the existence of one-way function \footnote{a one-way function is a function that is easy to compute on every input, but hard to invert given the set of all possible outputs of a random input, see: https://en.wikipedia.org/wiki/One-way\_function} can be proved one day, we can easily conclude that $\mathprob{MCSP}$ is not in $\P$ which implies the separation of $\P$ and $\NP$.
